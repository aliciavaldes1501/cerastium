...
p9<-ggplot()  +
  geom_smooth(data=interaction1,aes(x=FFD_std,y=fit_mod,group=as.factor(temp1),color=temp1),
              method=loess,se=F,size=0.3)+theme_base()+theme(text=element_text(family="serif"))+
  scale_color_gradient(low = "blue", high = "red")+ggtitle("B)")+
  theme(plot.background=element_rect(fill="white", colour=NA))+
...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/cerastium/results/figures/fitness_FFD_temp_2panels.tiff",
...
model9<-lmer(first_fl_j~temp_ori+(1|mother_pl_id_new),data = data_cg)
