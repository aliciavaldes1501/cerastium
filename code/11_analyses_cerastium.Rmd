...
p2<-ggplot(data_f_2015,aes(x=stream_new,fill=as.factor(flowered)))+geom_bar(position="dodge")+xlab("Stream")+ylab("Count")+my_theme()+theme(legend.position=c(0.8, 0.82),legend.title=element_blank())
...
p6<-ggplot(data_f_2015,aes(x=temp,y=flowered))+geom_point(aes(color=stream_new))+
  geom_smooth(aes(color=stream_new),method = "glm", method.args=c(family="binomial"),size=1,se=F,fullrange=F)+
  geom_smooth(method = "glm", method.args=c(family="binomial"),color="black",size=1.5)+
  my_theme()+xlab("Soil temperature")+ylab("Probability of having flowered\nat the day of recording")+
  # theme(legend.position=c(0.83,0.3))+
  ggtitle("A) 2015")+scale_x_continuous(limits=c(5,35))
p7<-ggplot(data_f_2017,aes(x=temp1,y=yday(FFD)))+geom_point(aes(color=plot_new))+
...
kable(prettify(summary(glm(flowered~temp,data=subset(data_f_2015,stream_new=="1"),family="binomial"))))
kable(prettify(summary(glm(flowered~temp,data=subset(data_f_2015,stream_new=="2"),family="binomial"))))
kable(prettify(summary(glm(flowered~temp,data=subset(data_f_2015,stream_new=="3"),family="binomial"))))
kable(prettify(summary(glm(flowered~temp,data=subset(data_f_2015,stream_new=="4"),family="binomial"))))
```
...
anova(model3,model4) # Likelihood ratio test comparing both models 
```

There is support for significant differences between slopes, so we keep model3 with different slopes and intercepts for each plot We cannot really separate the effects of plots from the effects of temperature, as plots have very different ranges of temperatures, i.e. effects of plots are likely to be mostly effects of temperatures. This means that differences in temperature at larger scales (among plots) are more important than differences at small scales (within plots). 
...
#Relative fitness and standardized traits
...
model8<-lmer(first_fl_j~stream_new+(1|mother_pl_id_new), REML=T,data=data_cg)
kable(prettify(summary(model8))) # Stream is significant
...
rand(model8) # Mother is significant
```
...
ggplot(data_cg,aes(x=temp_ori,y=first_fl_j))+geom_point(aes(color=stream_new))+
  geom_smooth(aes(color=stream_new),method = "lm", size=1,se=F)+
...
# This takes into account that mother is nested within stream
model9<-lmer(first_fl_j~temp_ori+(1|stream_new)+(1|mother_pl_id_new),data = data_cg)
kable(prettify(summary(model9))) # Temperature is significant
rand(model9) #Stream and mother are significant
```
